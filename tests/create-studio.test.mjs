...
for(const feature of ['readSheet','renderSheet','renderPresentation','currentSlides','data-pdf-text','evalFormula','toggleChart','pickSlideImage','resizeImage']) assert.ok(js.includes(feature),'missing working editor feature '+feature);
for(const formula of ['SUM','AVERAGE','MIN','MAX']) assert.ok(js.includes(formula),'missing local formula '+formula);
for(const control of ['data-table','data-block="H1"','data-chart','data-slide-layout','data-slide-image']) assert.ok(js.includes(control),'missing rich editor control '+control);
