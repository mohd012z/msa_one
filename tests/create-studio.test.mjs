...
for(const feature of ['normalizeSheets','replaceSheets','currentSheets','readSheet','renderSheet','renderPresentation','currentSlides','data-pdf-text','evalFormula','toggleChart','pickSlideImage','resizeImage','parseCSV','sanitizeHTML','importCurrent']) assert.ok(js.includes(feature),'missing working editor feature '+feature);
for(const formula of ['SUM','AVERAGE','MIN','MAX']) assert.ok(js.includes(formula),'missing local formula '+formula);
for(const control of ['data-table','data-block="H1"','data-chart','data-slide-layout','data-slide-image','data-import']) assert.ok(js.includes(control),'missing rich editor control '+control);
...
for(const ext of ['.docx','.xlsx','.pptx']) assert.ok(js.includes(ext),'missing Office import '+ext);
...
console.log('rich create studio contract passed');
