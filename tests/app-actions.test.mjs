...
for(const asset of ['theme-v2.css','create-studio.css','files-workspace.css','planner.css','settings-v2.js','create-studio.js','files-workspace.js','planner.js','app-actions.js']){
  assert.ok(html.includes(asset),'source HTML must load '+asset);
}
for(const action of ['reader','converter','automation','files','scan','image','voice','start','profile','ui-studio']){
  assert.ok(html.includes('data-msa-action="'+action+'"'),'missing wired action '+action);
}
for(const capability of ['MSAActions','SpeechRecognition',"type='file'",'MSAStudio','MSAPlanner','localStorage']){
