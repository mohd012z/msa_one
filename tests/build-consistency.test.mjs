...
assert.equal(pkg.version,'38.0.0','package version must match MSA One 38');
...
assert.equal(cap.appName,'MSA One 38','Capacitor app name must match build');
assert.equal(cap.appId,'com.msa.one.displayfit37','Android app ID must remain stable for in-place upgrades');
assert.ok(html.includes('data-build-id="MSA-ONE-38"'),'source UI build ID must match build');
assert.ok(workflow.includes('versionCode 38'),'Android versionCode must match build');
assert.ok(workflow.includes('MSA-One-38-APK'),'artifact name must match build');
assert.ok(!workflow.includes('Package Calendar and adaptive display UI'),'workflow must not mutate source UI before packaging');

console.log('MSA One 38 build consistency contract passed');
