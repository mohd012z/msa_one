...
assert.equal(pkg.version,'52.0.0','package version must match MSA One 52');
...
assert.equal(cap.appName,'MSA One 52','Capacitor app name must match build');
assert.equal(cap.appId,'com.msa.one.displayfit37','Android app ID must remain stable for in-place upgrades');
assert.ok(manifest.includes("version:'52.0.0'"),'app manifest version must match package');
assert.ok(manifest.includes("buildId:'MSA-ONE-52'"),'app manifest build ID must match UI');
assert.ok(premium.includes('active:false'),'Premium must remain inactive in Build 52');
assert.ok(premium.includes("billing:{\n      enabled:false"),'Billing must remain disabled in Build 52');
assert.ok(premium.includes("updatePolicy:{\n      enabled:true"),'remote update policy must be active in Build 52');
assert.ok(premium.includes('enforce:true'),'force update must be active in Build 52');
assert.ok(premium.includes("libraryVersion:'9.1.0'"),'prepared billing version must be 9.1.0');

assert.ok(html.includes('data-build-id="MSA-ONE-52"'),'source UI build ID must match build');
...
assert.ok(workflow.includes('versionCode 52'),'Android versionCode must match build');
assert.ok(workflow.includes('versionName "52.0"'),'Android versionName must match build');
assert.ok(workflow.includes('MSA-One-52-APK'),'artifact name must match build');
...
console.log('MSA One 52 Native Files consistency contract passed');
