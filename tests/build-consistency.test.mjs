...
assert.equal(pkg.version,'44.0.0','package version must match MSA One 44');
...
assert.equal(cap.appName,'MSA One 44','Capacitor app name must match build');
assert.equal(cap.appId,'com.msa.one.displayfit37','Android app ID must remain stable for in-place upgrades');

assert.ok(html.includes('data-build-id="MSA-ONE-44"'),'source UI build ID must match build');
for(const asset of ['library.css','helper.css','core-library.js','storage-engine.js','office-engine.js','import-engine.js','formula-engine.js','library-engine.js','helper-engine.js']){
...
assert.ok(workflow.includes('versionCode 44'),'Android versionCode must match build');
assert.ok(workflow.includes('versionName "44.0"'),'Android versionName must match build');
assert.ok(workflow.includes('MSA-One-44-APK'),'artifact name must match build');
for(const asset of ['core-library.js','library-engine.js','library.css','helper-engine.js','helper.css']){
...
console.log('MSA One 44 Friendly Helper consistency contract passed');
