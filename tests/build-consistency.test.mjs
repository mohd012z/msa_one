...
assert.equal(pkg.version,'53.0.0','package version must match MSA One 53');
...
assert.equal(cap.appName,'MSA One 53','Capacitor app name must match build');
assert.equal(cap.appId,'com.msa.one.displayfit37','Android app ID must remain stable for in-place upgrades');
assert.ok(manifest.includes("version:'53.0.0'"),'app manifest version must match package');
assert.ok(manifest.includes("buildId:'MSA-ONE-53'"),'app manifest build ID must match UI');
assert.ok(premium.includes('active:false'),'Premium must remain inactive in Build 53');
assert.ok(premium.includes("billing:{\n      enabled:false"),'Billing must remain disabled in Build 53');
assert.ok(premium.includes("updatePolicy:{\n      enabled:true"),'remote update policy must be active in Build 53');
assert.ok(premium.includes('enforce:true'),'force update must be active in Build 53');
assert.ok(premium.includes("libraryVersion:'9.1.0'"),'prepared billing version must be 9.1.0');

assert.ok(html.includes('data-build-id="MSA-ONE-53"'),'source UI build ID must match build');
for(const asset of ['library.css','helper.css','performance.css','premium.css','app-manifest.js','core-library.js','performance-engine.js','storage-engine.js','premium-config.js','entitlement-engine.js','version-policy.js','office-engine.js','import-engine.js','formula-engine.js','library-engine.js','library-updater.js','helper-engine.js','premium-ui.js','security-engine.js','native-file-bridge.js','workspace-v2.css','tools-catalog.js','template-catalog.js','assistant-catalog.js','action-sheet.js','drawer.js','search-center.js','home-v2.js','files-v2.js','tools-center.js','template-center.js','ai-tools.js','create-v2.js','app-shell.js']){
...
assert.ok(workflow.includes('versionCode 53'),'Android versionCode must match build');
assert.ok(workflow.includes('versionName "53.0"'),'Android versionName must match build');
assert.ok(workflow.includes('MSA-One-53-APK'),'artifact name must match build');
...
console.log('MSA One 53 Workspace UI consistency contract passed');
