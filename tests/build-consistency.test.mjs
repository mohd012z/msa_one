...
assert.equal(pkg.version,'47.0.0','package version must match MSA One 47');
...
assert.equal(cap.appName,'MSA One 47','Capacitor app name must match build');
assert.equal(cap.appId,'com.msa.one.displayfit37','Android app ID must remain stable for in-place upgrades');
assert.ok(manifest.includes("version:'47.0.0'"),'app manifest version must match package');
assert.ok(manifest.includes("buildId:'MSA-ONE-47'"),'app manifest build ID must match UI');
assert.ok(premium.includes('active:false'),'Premium must remain inactive in Build 47');
assert.ok(premium.includes('enabled:false'),'Billing/update policy must remain disabled in Build 47');
assert.ok(premium.includes('enforce:false'),'force update must remain disabled in Build 47');
assert.ok(premium.includes("libraryVersion:'9.1.0'"),'prepared billing version must be 9.1.0');

assert.ok(html.includes('data-build-id="MSA-ONE-47"'),'source UI build ID must match build');
for(const asset of ['library.css','helper.css','performance.css','premium.css','app-manifest.js','core-library.js','performance-engine.js','storage-engine.js','premium-config.js','entitlement-engine.js','version-policy.js','office-engine.js','import-engine.js','formula-engine.js','library-engine.js','library-updater.js','helper-engine.js','premium-ui.js']){
...
assert.ok(workflow.includes('versionCode 47'),'Android versionCode must match build');
assert.ok(workflow.includes('versionName "47.0"'),'Android versionName must match build');
assert.ok(workflow.includes('MSA-One-47-APK'),'artifact name must match build');
for(const asset of ['app-manifest.js','core-library.js','library-engine.js','library-updater.js','library.css','helper-engine.js','helper.css','performance.css','performance-engine.js','premium.css','premium-config.js','entitlement-engine.js','version-policy.js','premium-ui.js']){
...
console.log('MSA One 47 Premium Ready consistency contract passed');
