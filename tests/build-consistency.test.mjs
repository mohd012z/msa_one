...
assert.equal(pkg.version,'41.0.0','package version must match MSA One 41');
...
assert.equal(cap.appName,'MSA One 41','Capacitor app name must match build');
assert.equal(cap.appId,'com.msa.one.displayfit37','Android app ID must remain stable for in-place upgrades');
assert.ok(html.includes('data-build-id="MSA-ONE-41"'),'source UI build ID must match build');
...
assert.ok(workflow.includes('versionCode 41'),'Android versionCode must match build');
assert.ok(workflow.includes('versionName "41.0"'),'Android versionName must match build');
assert.ok(workflow.includes('MSA-One-41-APK'),'artifact name must match build');
assert.ok(workflow.includes('MSAImport'),'workflow must verify Office importer');
...
console.log('MSA One 41 build consistency contract passed');
