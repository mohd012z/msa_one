(()=> {
  ...
  function parseRef(ref){const m=String(ref).match(/^([A-Z]+)(\d+)$/i);if(!m)return null;let c=0;for(const ch of m[1].toUpperCase())c=c*26+ch.charCodeAt(0)-64;return {r:+m[2]-1,c:c-1}}
  ...
  function cellNumber(rows,r,c,seen=new Set()){
    const key=r+':'+c;if(seen.has(key))return 0;seen.add(key);const raw=String(rows[r]?.[c]??'').trim();let out=Number(raw);
    if(raw.startsWith('='))out=evalFormula(raw,rows,seen);seen.delete(key);return Number.isFinite(out)?out:0;
  }
  function evalFormula(formula,rows,seen=new Set()){
    ...
    expr=expr.replace(/\b(SUM|AVERAGE|MIN|MAX)\(([A-Z]+\d+):([A-Z]+\d+)\)/gi,(m,fn,a,b)=>{
      ...
      if(fn==='SUM')return String(vals.reduce((x,y)=>x+y,0));if(fn==='AVERAGE')return String(vals.reduce((x,y)=>x+y,0)/vals.length);if(fn==='MIN')return String(Math.min(...vals));return String(Math.max(...vals));
    });
    expr=expr.replace(/\b([A-Z]+)(\d+)\b/gi,(m,a,b)=>String(cellNumber(rows,+b-1,parseRef(a+b).c,seen)));
    if(!/^[0-9eE+\-*/().\s]+$/.test(expr))return NaN;
    try{const v=Function('"use strict";return ('+expr+')')();return Number(v)}catch{return NaN}
  }
