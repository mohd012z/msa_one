(()=> {
  ...
  function dataUrlAsset(dataUrl){
    ...
    return {ext,data:out,mime:ext==='png'?'image/png':'image/jpeg'};
  }
  function pptx(slides){
    slides=(slides&&slides.length?slides:[{title:'Untitled',body:'',layout:'title-body'}]);
    ...
    const files=[
      {name:'[Content_Types].xml',data:'<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Default Extension="png" ContentType="image/png"/><Default Extension="jpg" ContentType="image/jpeg"/><Default Extension="jpeg" ContentType="image/jpeg"/><Override PartName="/ppt/presentation.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.presentation.main+xml"/><Override PartName="/ppt/slideMasters/slideMaster1.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.slideMaster+xml"/><Override PartName="/ppt/slideLayouts/slideLayout1.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.slideLayout+xml"/><Override PartName="/ppt/theme/theme1.xml" ContentType="application/vnd.openxmlformats-officedocument.theme+xml"/>'+overrides+'</Types>'},
    ...
    slides.forEach((s,i)=>{
      const title=xml(s.title||'Slide '+(i+1)),body=xml(s.body||''),layout=s.layout||'title-body',asset=dataUrlAsset(s.image);
      const tx=(id,name,x,y,cx,cy,text,size,bold)=>'<p:sp><p:nvSpPr><p:cNvPr id="'+id+'" name="'+name+'"/><p:cNvSpPr txBox="1"/><p:nvPr/></p:nvSpPr><p:spPr><a:xfrm><a:off x="'+x+'" y="'+y+'"/><a:ext cx="'+cx+'" cy="'+cy+'"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom><a:noFill/></p:spPr><p:txBody><a:bodyPr/><a:lstStyle/><a:p><a:r><a:rPr lang="en-US" sz="'+size+'"'+(bold?' b="1"':'')+'><a:solidFill><a:srgbClr val="FFFFFF"/></a:solidFill></a:rPr><a:t>'+text+'</a:t></a:r><a:endParaRPr lang="en-US"/></a:p></p:txBody></p:sp>';
      let bodyX=700000,bodyY=2200000,bodyW=10800000,bodyH=3600000,img='';
      if(asset&&layout==='image-right'){bodyW=5000000;img='<p:pic><p:nvPicPr><p:cNvPr id="4" name="Image"/><p:cNvPicPr><a:picLocks noChangeAspect="1"/></p:cNvPicPr><p:nvPr/></p:nvPicPr><p:blipFill><a:blip r:embed="rId2"/><a:stretch><a:fillRect/></a:stretch></p:blipFill><p:spPr><a:xfrm><a:off x="6200000" y="1900000"/><a:ext cx="5200000" cy="3900000"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></p:spPr></p:pic>'}
      if(asset&&layout==='image-full'){bodyY=5700000;bodyH=700000;img='<p:pic><p:nvPicPr><p:cNvPr id="4" name="Image"/><p:cNvPicPr><a:picLocks noChangeAspect="1"/></p:cNvPicPr><p:nvPr/></p:nvPicPr><p:blipFill><a:blip r:embed="rId2"/><a:stretch><a:fillRect/></a:stretch></p:blipFill><p:spPr><a:xfrm><a:off x="700000" y="1700000"/><a:ext cx="10800000" cy="3800000"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></p:spPr></p:pic>'}
      const slideXml='<?xml version="1.0" encoding="UTF-8" standalone="yes"?><p:sld xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main"><p:cSld><p:bg><p:bgPr><a:solidFill><a:srgbClr val="081523"/></a:solidFill><a:effectLst/></p:bgPr></p:bg><p:spTree><p:nvGrpSpPr><p:cNvPr id="1" name=""/><p:cNvGrpSpPr/><p:nvPr/></p:nvGrpSpPr><p:grpSpPr><a:xfrm/></p:grpSpPr>'+tx(2,'Title',700000,500000,10800000,1200000,title,2800,true)+tx(3,'Body',bodyX,bodyY,bodyW,bodyH,body,1600,false)+img+'</p:spTree></p:cSld></p:sld>';
      files.push({name:'ppt/slides/slide'+(i+1)+'.xml',data:slideXml});
      let rel='<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideLayout" Target="../slideLayouts/slideLayout1.xml"/>';
      if(asset){
        const media='image'+(i+1)+'.'+asset.ext;
        files.push({name:'ppt/media/'+media,data:asset.data});
        rel+='<Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/'+media+'"/>';
      }
      rel+='</Relationships>';
      files.push({name:'ppt/slides/_rels/slide'+(i+1)+'.xml.rels',data:rel});
    });
    return zip(files);
  }
