...
function persist(k,v){localStorage.setItem(k,v);window.MSAStorage?.mirror(k,v)}function setShape(k){persist('msaButtonShape',S[k]?k:'soft');refresh()}function setEffect(k){persist('msaButtonEffect',E[k]?k:'gradient');refresh()}function setIntensity(v){persist('msaButtonIntensity',String(v));refresh()}
